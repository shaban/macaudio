...
func (b *Bus) ConnectChannel(ch Channel) (int, error) {
    ...
    if installed, err := node.IsInstalledOnEngine(src); err == nil && !installed {
        if err := b.eng.Attach(src); err != nil { return -1, fmt.Errorf("attach source: %w", err) }
    }
...
func NewMasterBus(eng *engine.Engine) (*MasterBus, error) {
    if eng == nil { return nil, fmt.Errorf("engine instance cannot be nil") }
    mm, err := eng.MainMixerNode()
    if err != nil || mm == nil { return nil, fmt.Errorf("main mixer: %v", err) }
